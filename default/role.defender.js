...
patrol: function(creep) {
    const flags = creep.room.find(FIND_FLAGS).filter(flag => flag.name.startsWith("Patrol"));
    if(flags.length > 0) {
        if(!creep.memory.patrolPoint || creep.memory.patrolPoint >= flags.length) {
        ...
        if(!creep.pos.inRangeTo(targetFlag, 3)) { // Consider being within 3 squares close enough
        ...
        creep.moveTo(Game.spawns['Spawn1'].pos.x + 5, Game.spawns['Spawn1'].pos.y, {visualizePathStyle: {stroke: '#ffffff'}});
    }
}
